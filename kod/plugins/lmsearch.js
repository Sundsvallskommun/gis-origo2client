var Lmsearch = (function(Origo2) {
  ...
  var hasRequiredAwesomplete;
  function requireAwesomplete() {
    if (hasRequiredAwesomplete) return awesomplete.exports;
    hasRequiredAwesomplete = 1;
    (function(module) {
      (function() {
        var _2 = function(input, o) {
          var me = this;
          _2.count = (_2.count || 0) + 1;
          this.count = _2.count;
          this.isOpened = false;
          this.input = $(input);
          ...
          configure(this, {
            ...
            autoFirst: false,
            data: _2.DATA,
            filter: _2.FILTER_CONTAINS,
            sort: o.sort === false ? false : _2.SORT_BYLENGTH,
            container: _2.CONTAINER,
            item: _2.ITEM,
            replace: _2.REPLACE,
            tabSelect: false,
        ...
        _2.prototype = {
          ...
          destroy: function() {
            ...
            var indexOfAwesomplete = _2.all.indexOf(this);
            if (indexOfAwesomplete !== -1) {
              _2.all.splice(indexOfAwesomplete, 1);
            }
          },
          next: function() {
        ...
        _2.all = [];
        _2.FILTER_CONTAINS = function(text, input) {
          return RegExp($.regExpEscape(input.trim()), "i").test(text);
        };
        _2.FILTER_STARTSWITH = function(text, input) {
          return RegExp("^" + $.regExpEscape(input.trim()), "i").test(text);
        };
        _2.SORT_BYLENGTH = function(a, b) {
          if (a.length !== b.length) {
            return a.length - b.length;
          }
          return a < b ? -1 : 1;
        };
        _2.CONTAINER = function(input) {
          return $.create("div", {
            className: "awesomplete",
            around: input
          });
        };
        _2.ITEM = function(text, input, item_id) {
        ...
        _2.REPLACE = function(text) {
          this.input.value = text.value;
        };
        _2.DATA = function(item) {
          return item;
        };
  ...
  function prepCommuneCodes(municipalities) {
    ...
    for (let i = 0; i < municipalitiesTokenized.length; i += 1) {
      results.push(communeCodes$1.getCommuneCode(municipalitiesTokenized[i].trim()));
    }
    return results.join(",");
  }
  ...
  const generateUUID = function generateUUID2() {
  ...
  const Main = function Main2(options = {}) {
    const {
      viewer,
      geometryAttribute,
      layerNameAttribute,
    ...
    let target;
    const vectorStyles = Origo2.Style.createStyleRule(featureStyles);
    ...
    const iconStyle = new Origo2.ol.style.Style({
      image: new Origo2.ol.style.Icon({
    ...
    const setEstateActive = function setEstateActive2(state) {
      if (state) {
        estateLookupOn = true;
    ...
    const clearFeatureInfo = function clearFeatureInfo2() {
      featureInfo.clear();
      if (overlay) {
        viewer.removeOverlays(overlay);
      }
    };
    const clearVectorFeatures = function clearVectorFeatures2() {
      vectorSource.clear();
    };
    const fetchFastighetsYtaById = function fetchFastighetsYtaById2(objectId) {
      const urlTemp = urlYta.replace("objectId", objectId);
      return fetch(urlTemp).then((response) => {
        if (!response.ok) {
    ...
    const readEstateFeatures = function readEstateFeatures2(response) {
      const format = new Origo2.ol.format.GeoJSON();
      ...
      if (features.length > 1) {
        ...
        if (features[0].getGeometry().getType() === "Polygon") {
          const multiGeom = new Origo2.ol.geom.MultiPolygon([]);
    ...
    const createEstateReport = function createEstateReport2(feature, objectId) {
    ...
    const showFeatureInfo = function showFeatureInfo2(features, objTitle, contentFeatureInfo, objectId) {
      if (showFeature === "popup") {
        ...
        clearVectorFeatures();
        vectorSource.addFeature(new Origo2.ol.Feature({
          geometry: features[0].getGeometry(),
          name: objTitle
        }));
        if (typeof objectId !== "undefined" && pageEstateReportUrl !== "") {
          const iconFeature = new Origo2.ol.Feature({
            geometry: new Origo2.ol.geom.Point(viewer.getMapUtils().getCenter(features[0].getGeometry())),
    ...
    const showEstateByObjectId = function showEstateByObjectId2(objectId, objTitle, contentHtml) {
    ...
    const setSearchFieldValue = function setSearchFieldValue2(value) {
    ...
    const bindPartOwnerLinks = function bindPartOwnerLinks2(modal) {
    ...
    return Origo2.ui.Component({
      ...
      onInit() {
        ...
        northing = options.northing || void 0;
        easting = options.easting || void 0;
        this.geometryAttribute = options.geometryAttribute;
        idAttribute = options.idAttribute;
        this.layerNameAttribute = options.layerNameAttribute || void 0;
        layerName = options.layerName || void 0;
        options.url;
        this.title = options.title || "";
        this.titleAttribute = options.titleAttribute || void 0;
        this.contentAttribute = options.contentAttribute || void 0;
        ...
        prepOptions.elasticSearch = options.elasticSearch || void 0;
        prepOptions.municipalities = options.municipalities;
        prepOptions.statusAddress = options.statusAddress ? options.statusAddress : void 0;
        urlYta = options.urlYta;
        urlYtaKord = options.urlYtaKordinat;
        vectorSource = new Origo2.ol.source.Vector();
        vectorLayer = new Origo2.ol.layer.Vector({
          source: vectorSource,
          style: vectorStyles
        });
        ...
        if (estateLookup && !(isEmbedded && hideWhenEmbedded)) {
          estateButton = Origo2.ui.Button({
            ...
            click() {
              ...
              viewer.dispatch("toggleClickInteraction", detail2);
            },
      ...
      render() {
        if (searchEnabled) {
          ...
          let elLayerManger = Origo2.ui.dom.html(template);
          document.getElementById(viewer.getMain().getId()).appendChild(elLayerManger);
          elLayerManger = Origo2.ui.dom.html(template);
        }
        if (estateLookup && !(isEmbedded && hideWhenEmbedded)) {
          const htmlString = estateButton.render();
          const el = Origo2.ui.dom.html(htmlString);
      ...
      initAutocomplete() {
        ...
        awesomplete2 = new Awesomplete("#o-lmsearch .o-search-field", {
          minChars: minLength,
          autoFirst: false,
          sort: false,
          maxItems: limit,
        ...
        const delay = /* @__PURE__ */ (function delay2() {
          let timer = 0;
      ...
      selectHandler(evt) {
        ...
        function showOverlay(dataOverlay, coordOverlay) {
          clearFeatureInfo();
          const newPopup = Origo2.popup("#o-map");
          overlay = new Origo2.ol.Overlay({
          ...
          viewer.zoomToExtent(new Origo2.ol.geom.Point(coordOverlay), maxZoomLevel);
        }
        if (layerNameAttribute && idAttribute) {
          layer = viewer.getLayer(data[layerNameAttribute]);
          id = data[idAttribute];
          this.getFeature(id, layer).then((res) => {
            let featureWkt;
            let coordWkt;
            if (res.length > 0) {
          ...
          showFeatureInfo([feature], layer.get("title"), Origo2.getAttributes(feature, layer));
      ...
      onMapClick(evt) {
        function fetchFastighetsYta(coords) {
          ...
          return fetch(urlTemp).then((response) => response.json());
        }
          ...
          if (showFeature === "geometryOnly") {
            if (features.length > 1) {
              features.forEach((feature) => {
                ...
                const newStyle = new Origo2.ol.style.Style({
                  text: new Origo2.ol.style.Text({
                    text: strEstate,
                    font: labelFont,
                    stroke: new Origo2.ol.style.Stroke({
                      color: labelFontColor
                    }),
                    backgroundFill: new Origo2.ol.style.Fill({
                      color: labelBackgroundColor
                    }),
                    overflow: true
                  })
                });
                const newFeature = new Origo2.ol.Feature();
                newFeature.setGeometry(new Origo2.ol.geom.Point(viewer.getMapUtils().getCenter(feature.getGeometry())));
            ...
            const clickStyle = new Origo2.ol.style.Style({
              text: new Origo2.ol.style.Text({
            ...
            const clickFeature = new Origo2.ol.Feature();
            clickFeature.setGeometry(new Origo2.ol.geom.Point(coordinate));
            ...
            if (typeof features[0].getProperties().objektidentitet !== "undefined" && pageEstateReportUrl !== "") {
              ...
              const iconFeature = new Origo2.ol.Feature({
                geometry: new Origo2.ol.geom.Point(coordinate),
                // Set the geometry of the estate report icon
                name: localization.getStringByKeys({ targetParentKey: "lmsearch", targetKey: "communityAssociationTitle" }),
        ...
        map.forEachFeatureAtPixel(
          ...
          (feature) => {
            if (typeof feature.getProperties().pageEstateReport !== "undefined" && pageEstateReportUrl !== "") {
              const estateReportModal = Origo2.ui.Modal({
        ...
        if (estateLookupOn && !estateInfoClick) {
          ...
          areaPromise.then((response) => {
            ...
            if (features.length > 1 && showFeature === "popup") {
              ...
              features.forEach((feature) => {
                if (feature.getGeometry().getType() === "Polygon") {
                  const polygon = new Origo2.ol.geom.Polygon(feature.getGeometry().getCoordinates());
            ...
            featureName = featureName.substring(0, featureName.indexOf("Enhetsomr"));
            const contentArr = [];
            contentArr.push('<div class="o-identify-content">');
            contentArr.push(viewer.getUtils().createElement("p", `${featureName}`));
            Object.entries(features[0].getGeometry().getProperties()).forEach((prop) => {
              contentArr.push(viewer.getUtils().createElement("p", `<b>${prop[0]}:</b><br/> ${prop[1]}`));
            });
            contentArr.push("</div>");
            showClickedFeatureInfo(features, localization.getStringByKeys({ targetParentKey: "lmsearch", targetKey: "showFeatureInfoTitle" }), contentArr.join(""), coordinate);
          }).catch((err) => {
            console.log(err);
          });
        }
      ...
      bindUIActions() {
        if (searchEnabled) {
          ...
          document.querySelector(".o-search-field").addEventListener("blur", () => {
            document.querySelector(".o-search-wrapper").classList.remove("active");
            window.dispatchEvent(new Event("resize"));
          });
          document.querySelector(".o-search-field").addEventListener("focus", () => {
            document.querySelector(".o-search-wrapper").classList.add("active");
            window.dispatchEvent(new Event("resize"));
          });
        }
      ...
      onClearSearch() {
        document.querySelector("#o-lmsearch-button-close").addEventListener("click", (e) => {
          ...
          document.querySelector("#o-lmsearch").classList.remove("o-search-true");
          document.querySelector("#o-lmsearch").classList.add("o-search-false");
          document.querySelector("#o-lmsearch .o-search-field").value = "";
          document.querySelector("#o-lmsearch-button").blur();
          e.preventDefault();
        });
      }
    });
  ...
  const Lmsearch2 = function Lmsearch3(options = {}) {
    const {
      geometryAttribute,
      // Attribute for geometry
      layerNameAttribute,
      // Attribute for layer name
      searchAttribute,
      // Attribute used for performing searches
      titleAttribute,
      // Attribute for displaying titles
      contentAttribute,
      // Attribute for displaying detailed content
      title,
      // Title of the search component
      hintText,
      // Hint text for the user (e.g., "Search for an address")
      searchLabelText,
      // Label text next to the search field
      estateLookup,
      // Whether property lookup is enabled
      estateLookupInitialState,
      // Initial state for property lookup
      minLength,
      // Minimum number of characters to start a search
      limit,
      // Maximum number of search results to return
      maxZoomLevel,
      // Maximum zoom level to display search results
      municipalities,
      // List of municipalities to include in the search
      statusAddress,
      // Whether to include address status in search results
      showFeature,
      // Whether to display the search result as a feature on the map
      featureStyles,
      // Styles for displaying search results as features
      labelFont,
      // Font for labels
      labelFontColor,
      ...
      urlFastighet,
      // URL for property data
      urlAdress,
      // URL for address data
      urlOrt,
      // URL for locality data
      urlYta,
      // URL for area data to include
      urlYtaKordinat,
      // URL for coordinates representing areas
      elasticSearch,
      // Configuration for ElasticSearch integration
      pageEstateReportWidth,
      // Width for estate report window
      pageEstateReportHeight,
      // Height for estate report window
      pageEstateReportUrl,
      // URL for generating property reports
      pageEstateIconText,
      // Text displayed in estate report icons
      pageEstateIconSize,
      // Size of the estate report icon
      searchEnabled,
      // Whether the search functionality is enabled
      hideWhenEmbedded
      // Whether to hide the component when embedded
    } = options;
    let viewer;
    let search;
    return Origo2.ui.Component({
      ...
      onAdd(evt) {
        ...
        localization.addPluginToLocale("fi-FI", fiLocale);
        localization.addPluginToLocale("nl-NL", nlLocale);
        localization.addPluginToLocale("sma", smaLocale);
        search = Main({
          viewer,
          // Reference to the Origo viewer
          geometryAttribute,
          layerNameAttribute,
          searchAttribute,
          titleAttribute,
...
//# sourceMappingURL=lmsearch.js.map
