...
function addAudioListener() {
  const fn = (obj) => {
    const fileReader = new FileReader();
    const containerElement = document.querySelector(`.${obj.elId.slice(1)}`);

    if (!containerElement) return;
    const inputElement = document.querySelector(obj.elId);
    const audioElement = containerElement.querySelector('audio');
    const buttonElement = containerElement.querySelector('input[type=button]');
    const sourceElement = containerElement.querySelector('source');

    inputElement.addEventListener('change', (ev) => {
      console.log('addAudioListener origo');
      if (ev.target.files && ev.target.files[0]) {
    ...
    if (editableAttributes.length > 0) {
      ...
      /* (async () => {
        try {
          ...
          const adress = response.data.features[0].properties.adressomrade.faststalltNamn;
          const adressnr = response.data.features[0].properties.adressplatsattribut.adressplatsbeteckning.adressplatsnummer;
          const postnummer = response.data.features[0].properties.adressplatsattribut.postnummer;
          const postort = response.data.features[0].properties.adressplatsattribut.postort;
          console.log(`${adress} ${adressnr}, ${postnummer} ${postort}`);
          lookupValue = `${adress} ${adressnr}, ${postnummer} ${postort}`;
        } catch (error) {
          console.log(error);
        }
      })(); */
      ...
      attributeObjects = editableAttributes.map((attributeObject) => {
        ...
        if (obj.type === 'lookup') {
          ...
          console.log(obj.val);
        } else if (obj.defaultValue && obj.defaultValue.updateOnEdit) {
