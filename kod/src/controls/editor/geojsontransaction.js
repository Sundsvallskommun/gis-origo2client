...
function writeGeojsonTransaction(features, options) {
  const data = {};
...
function geojsonTransaction(transObj, layerName, viewer) {
  ...
  types.forEach((type) => {
    if (transObj[type]) {
      ...
      const data = writeGeojsonTransaction(transObj[type], {
        projection,
        type
      });
      fetch(url, {
...
export default function geojsontransaction(transObj, layerName, viewer) {
  return geojsonTransaction(transObj, layerName, viewer);
}
